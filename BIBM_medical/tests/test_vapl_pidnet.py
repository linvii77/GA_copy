...
def test_compositional_similarity_loss_use_variation_false_clean_degeneration_cpu() -> None:
    """use_variation=False must be a clean single-Gaussian (SCDL-style) proxy:
    no variation_vectors parameter at all, and combined = q_c directly."""
    torch.manual_seed(0)
    loss_fn = CompositionalSimilarityLoss(
        num_classes=4, embedding_dim=8, num_variations=3, use_variation=False
    )

    assert loss_fn.variation_vectors is None
    assert "variation_vectors" not in dict(loss_fn.named_parameters())
    ...
    # combined = q_c: the positive probability for each pixel must equal
    # q at its target class, with no p_sub term involved.
    resized_targets = loss_fn._resize_targets(targets, embeddings.shape[2:])
    flat_embeddings, flat_targets = loss_fn._flatten_valid(embeddings, resized_targets)
    x = torch.nn.functional.normalize(flat_embeddings, p=2, dim=1)
    q, _ = loss_fn._proxy_assignment_probabilities(x)
    arange = torch.arange(flat_targets.numel())
    expected_p_pos = q[arange, flat_targets].clamp_min(loss_fn.eps)
    expected_loss_attraction = -torch.log(expected_p_pos).mean()
    assert torch.allclose(stats.loss_attraction, expected_loss_attraction, atol=1e-6)


def test_compositional_similarity_loss_variation_active_false_matches_use_variation_false_cpu() -> None:
    """variation_active=False (with use_variation=True) must behave exactly
    like the clean use_variation=False degeneration: combined=q_c, and
    variation_vectors receives no gradient (variation warmup schedule)."""
    torch.manual_seed(0)
    loss_fn = CompositionalSimilarityLoss(num_classes=4, embedding_dim=8, num_variations=3)
    loss_fn.variation_active = False

    assert loss_fn.variation_vectors is not None
    ...
    assert loss_fn.proxy_dist.grad is not None
    assert loss_fn.variation_vectors.grad is None
