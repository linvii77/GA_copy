...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable the variation sub-distribution (clean single-Gaussian "
            "SCDL-style proxy ablation: combined = q_c, no variation_vectors "
            "parameter)."
        ),
    )
    parser.add_argument(
        "--variation-warmup-steps",
        type=int,
        default=0,
        help=(
            "If use_variation=True, behave as combined=q_c (no p_sub term, "
            "no gradient to variation_vectors) for this many initial steps, "
            "then activate the variation sub-distribution. No-op with "
            "--no-variation."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load model weights only (strict=False, no optimizer/scaler/"
            "iteration/best_dice state) from a checkpoint. Useful for "
            "warm-starting a differently-configured model, e.g. comparing "
            "use_variation=True/False from the same pretrained backbone."
        ),
    )
...
def main() -> None:
    ...
    if args.init_weights is not None:
        load_pretrained_weights(args.init_weights, model)
    if args.variation_warmup_steps > 0 and model.cs_loss.use_variation:
        model.cs_loss.variation_active = False
    ...
    for iteration in range(start_iter, args.max_iters):
        if (
            args.variation_warmup_steps > 0
            and model.cs_loss.use_variation
            and not model.cs_loss.variation_active
            and iteration >= args.variation_warmup_steps
        ):
            model.cs_loss.variation_active = True
            print(
                f"activated variation sub-distribution at iter={iteration + 1}",
                flush=True,
            )
...
def load_pretrained_weights(path: Path, model: torch.nn.Module) -> None:
    checkpoint = torch.load(path, map_location="cpu", weights_only=False)
    missing, unexpected = model.load_state_dict(checkpoint["model"], strict=False)
    print(
        f"loaded weights from {path} "
