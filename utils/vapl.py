"""Variation-Aware Proxy Learning modules (ported from BIBM_medical idea project)."""
...
class CompositionalSimilarityLoss(nn.Module):
    """VAPL compositional similarity loss with per-class Gaussian proxies."""
    ...
    def __init__(
        ...
        self.proxy_dist = nn.Parameter(torch.empty(num_classes, embedding_dim * 2))
    ...
    def forward(
        ...
        if flat_embeddings.numel() == 0:
            ...
            stats = VAPLStats(
                loss_cs=zero, loss_attraction=zero, loss_repulsion=zero,
                positive_probability=zero, negative_probability=zero,
                hard_fraction=zero,
                valid_pixels=torch.zeros((), device=embeddings.device),
                proxy_assignment_accuracy=zero, proxy_sigma_mean=zero,
            )
            return zero, stats
        ...
        stats = VAPLStats(
            loss_cs=loss_cs, loss_attraction=loss_attraction, loss_repulsion=loss_repulsion,
    ...
    def _proxy_params(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return mu, sigma.clamp_min(self.proxy_sigma_min)
    ...
    def _proxy_assignment_probabilities(
        ...
        return torch.softmax(proxy_logits, dim=1), sigma_c
