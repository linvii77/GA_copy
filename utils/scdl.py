"""SCDL-style semantic class distribution learning modules (ported from BIBM_medical)."""
...
class SemanticClassDistributionLoss(nn.Module):
    ...
    Takes raw backbone features ([B, C, D, H, W]) and projects them internally.
    No external ProjectionHead is needed.
    """
    ...
    def forward(
        ...
        if flat_embeddings.numel() == 0:
            ...
            stats = SCDLStats(
                loss_scdl=zero, loss_scdl_align=zero, loss_scdl_proxy=zero,
                loss_scdl_anchor=zero, scdl_true_probability=zero.detach(),
        ...
        loss_scdl = (
            loss_align
            + self.proxy_loss_weight * loss_proxy
            + self.anchor_loss_weight * loss_anchor
        )

        prior_map = self._prior_map(embeddings, mu_norm)
        stats = SCDLStats(
            loss_scdl=loss_scdl, loss_scdl_align=loss_align, loss_scdl_proxy=loss_proxy,
    ...
    def _sample_proxies(self, mu: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        ...
        return F.normalize(mu.unsqueeze(1) + sigma.unsqueeze(1) * noise, p=2, dim=-1)
    ...
    def _anchor_loss(
        ...
        for class_index in torch.unique(flat_targets):
            ...
            if not mask.any():
                continue
            center = F.normalize(flat_embeddings[mask].mean(dim=0), p=2, dim=0)
            losses.append(1.0 - torch.sum(center * mu_norm[class_index]))
        if not losses:
    ...
    def _prior_map(self, embeddings: torch.Tensor, mu_norm: torch.Tensor) -> torch.Tensor:
        flat_embeddings = embeddings.movedim(1, -1).reshape(-1, embeddings.shape[1])
        probs = torch.softmax(
            torch.matmul(flat_embeddings, mu_norm.t()) / self.tau_p2e, dim=1
        )
        prior_tokens = torch.matmul(probs, mu_norm)
        prior = prior_tokens.view(
            *embeddings.shape[0:1], *embeddings.shape[2:], self.embedding_dim
        )
